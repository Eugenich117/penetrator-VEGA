...
J4        = -1.0e-5        # четвертый зональный коэффициент (C40); отрицателен для Луны (GRGM1200A)
...
# Секториальные C22/S22 дают нулевой вклад на полюсе: P₂²(±1) = 0.
# Расчёт учитывает это через P22 = 3*(1 - cos²θ).
...
MASCONS = [
    ...
    # SPA — крупнейший ударный бассейн Луны (диаметр ~2500 км).
    # По данным GRAIL: SPA не заполнен лавой, гравитационная аномалия —
    # преимущественно дефицит массы (dm < 0). Отрицательный знак dm корректен.
    ...
    # ПРИМЕЧАНИЕ: кратеры южной полярной области (Shackleton, Shoemaker,
    # de Gerlache, Faustini и др.) УДАЛЕНЫ из списка масконов.
    # Причина: ударные кратеры без лавового заполнения — НЕ масконы.
    # Масконы = моря, заполненные базальтовой лавой (избыток плотной массы).
    # Полярные кратеры создают топографический дефицит, а не избыток массы.
    # Их гравитационный вклад учтён через зональные гармоники (J2–J4).
]
...
OMEGA_MOON = 2.662e-6  # рад/с (угловая скорость синхронного вращения Луны)
# Реальная либрация (период ~27.3 сут) пренебрежимо мала на масштабе посадки (~500 с).
# «Быстрая либрация» с периодом ~100 с физически не существует и удалена.
...
def g_func(h, lat=-90.0, lon=0.0, t=0.0):
    ...
    Зональные гармоники включают полиномы Лежандра P_n(cos θ), θ = 90° − lat.
    Секториальные C22/S22 обнуляются на полюсе через P₂²(cos θ) = 3(1−cos²θ).
    """
    h_eff = max(h, 0.0)
    r = R_moon + h_eff
    ...
    # ── Зональные возмущения (J2, J3, J4) с полиномами Лежандра ─────
    # cos_theta = sin(lat): для южного полюса lat=-90° → cos_theta=-1
    # P2(-1)=1, P3(-1)=-1, P4(-1)=1 — знаки важны!
    cos_theta = np.sin(np.radians(lat))
    P2 = 0.5  * (3.0 * cos_theta**2 - 1.0)
    P3 = 0.5  * (5.0 * cos_theta**3 - 3.0 * cos_theta)
    P4 = 0.125 * (35.0 * cos_theta**4 - 30.0 * cos_theta**2 + 3.0)

    delta_J2 = 3.0 * J2 * r_ratio ** 2 * P2
    delta_J3 = 4.0 * J3 * r_ratio ** 3 * P3
    delta_J4 = 5.0 * J4 * r_ratio ** 4 * P4

    # ── Секториальные возмущения (C22, S22) ─────────────────────────
    # P22(cos_theta) = 3*(1 - cos_theta²); на полюсе cos_theta=±1 → P22=0
    P22 = 3.0 * (1.0 - cos_theta**2)
    lon_eff = lon + np.degrees(OMEGA_MOON * t)   # только реальное вращение Луны
    lon_rad = np.radians(lon_eff)
    delta_C22 = 6.0 * C22 * r_ratio ** 2 * P22 * np.cos(2.0 * lon_rad)
    delta_S22 = 6.0 * S22 * r_ratio ** 2 * P22 * np.sin(2.0 * lon_rad)

    # ── Возмущения от масконов ──────────────────────────────────────
    # Модель точечных масс: Δg_r = G·Δm·(r − r_mc·cosψ) / d³
    delta_mascons = 0.0

    # Координаты КА (lon_rad уже вычислен выше)
    ...
    for mascon in MASCONS:
        ...
        # Радиальная компонента: Δg_r = G·Δm·(r − r_mc·cosψ) / d³
        # Знак dm: >0 — избыток массы (моря), <0 — дефицит (SPA)
        # clip убран: radial_factor всегда в [0,1] при r > r_mc, но знак dm важен
...
def g_mascon_component(h, lat=-90.0, lon=0.0, t=0.0):
    ...
    r = R_moon + h_eff
    lat_rad = np.radians(lat)
    lon_eff = lon + np.degrees(OMEGA_MOON * t)   # согласовано с g_func
    lon_rad = np.radians(lon_eff)
