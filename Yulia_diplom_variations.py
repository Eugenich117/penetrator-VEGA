...
def M(R):
    ...
    y = [0.0021298, 0.0021323, 0.0021348, 0.0021373, 0.0021398, 0.0021423, 0.0021448, 0.0021473, 0.0021498, 0.0021523,
         ...
         0.0023048, 0.0023065, 0.0023082, 0.0023099, 0.0023116, 0.0023133, 0.0023150, 0.0023167, 0.0023184, 0.0023201,
         0.0023218, 0.0023235, 0.0023252, 0.0023269, 0.0023286, 0.0023303, 0.0023320, 0.0023337, 0.0023354, 0.0023371,
         0.0023388, 0.0023405, 0.0023422, 0.0023439, 0.0023456, 0.0023473, 0.0023490, 0.0023507, 0.0023524]
...
def calc_U(ro):
    P_He = 0.4 * 100000  # давление гелия в Па (0.3306 бар)
    M_He = 0.004  # молярная масса гелия в кг/моль
    R = 8.314  # универсальная газовая постоянная в Дж/(моль·К)
    T = 320  # температура в К

    # Плотность гелия при данных условиях
    rho_He = (P_He * M_He) / (R * T) # Плотность гелия в кг/м^3
    P_in = 150 * 100000  # Давление внутри баллона в Па (150 бар)
    P_out = ro * 100000  # Внешнее давление в Па (0.33 бар)
    d = 0.015  # диаметр шланга в метрах
    # Изменение объема за время dt
    U = m.pi * (d / 2) ** 2 * m.sqrt(2 * (P_in - P_out) / rho_He) * dt
    return U
...
def compute_trajectory(i, equations, dx, pipe_conn):
    try:
        ...
        while t <= t_stop:
            ...
            local_nx.append((0.5 * S * Cxa * ro * V ** 2) / (mass * ((gravy_const * mass_planet) / R ** 2)))
            local_PX.append(Px)
        print(f'7) V = {V:.3f}, tetta = {tetta * cToDeg:.3f}, L = {L:.3f}, H = {(R - Rb):.3f}, t = {t:.3f}, U = {U:.3f}')'''
